...
```{r}
cities <- cities %>%
  mutate(dist_to_mexico = set_units(st_distance(., mexico_border), "km")) %>%
  mutate(dist_to_canada = set_units(st_distance(., canada_border), "km")) %>%
  mutate(dist_to_border = set_units(st_distance(., state_union), "km"))
...
ggplot() + 
  ...
  geom_sf(data = conus, size = 1.5, fill = "NA", color = "black") + 
  geom_sf(data = eqdist) + 
